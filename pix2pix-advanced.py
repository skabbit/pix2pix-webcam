...
parser.add_argument("--no_input", action="store_true", help="only targets given, input generated on-fly (see Memo original article)")
parser.set_defaults(no_input=True)
...
def load_examples():
    ...
    def transform(image, process=False):
        r = image
        if a.flip:
        ...
        if process:
            ...
            r = tf.image.resize_images(r, [DOWNSCALE_SIZE, DOWNSCALE_SIZE], method=tf.image.ResizeMethod.AREA)
            r = tf.image.resize_images(r, [CROP_SIZE, CROP_SIZE], method=tf.image.ResizeMethod.BICUBIC)
            
            r = deprocess(r)
            # apply random brightness of up to +/- 20%
            r = tf.image.random_brightness(r, 0.20)

            # apply random contrast of up to +/- 15%
            r = tf.image.random_contrast(r, 0.85, 1.15)
            r = preprocess(r)

            # clip to prevent distortions
            r = tf.clip_by_value(r, -1, 1)

        return r

    with tf.name_scope("input_images"):
        input_images = transform(targets, process=True)
